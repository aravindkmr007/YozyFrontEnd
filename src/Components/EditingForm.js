...
function EditingForm({ FormOpen, setFormOpen ,Selection,getSelection,Values,setValues}) {
  const initialValue = {
    firstname: "",
    lastname: "",
    gender : "",
  ...
  const EditingEmpolyee = (e) => {
    e.preventDefault();
    if (validate()) {
      axios.patch('http://localhost:4000/Empolyee/' + Values.id,Values)
      setValues(initialValue)
